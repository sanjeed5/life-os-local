...
2.  **Gather**: Read the last 7 days from `journal/`. If the week spans a month boundary, check both the current and previous month's files.
...
6.  **Inbox Audit**: Skim `inbox.md`. Ask if any items should be promoted to next week's focus or archived to "Someday".
